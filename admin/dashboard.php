...
// Statistik per bulan (menggantikan statistik per minggu)
$stats_sql = "SELECT 
    DATE_FORMAT(uploaded_at, '%Y-%m') AS month_year,
    DATE_FORMAT(uploaded_at, '%b %Y') AS formatted_month, 
    COUNT(*) AS count
FROM documents
WHERE uploaded_at >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
GROUP BY month_year
ORDER BY month_year";
$monthlyStats = $pdo->query($stats_sql)->fetchAll();

// Statistik dokumen per user
$userStats = $pdo->query("
    SELECT u.id, u.username, u.full_name, COUNT(d.id) AS doc_count
    FROM users u
...
<body>
            ...
            <div class="content">
                ...
                <div class="card">
                    <div class="card-header">
                        <h2>Statistik Pengajuan 12 Bulan Terakhir</h2>
                    </div>
                ...
                <!-- TABEL STATISTIK DOKUMEN PER USER -->
                <div class="card">
                    <div class="card-header">
                        <h2>Statistik Dokumen Tiap User</h2>
                    </div>
    ...
    <script>
        ...
        const monthLabels = <?= json_encode(array_column($monthlyStats, 'formatted_month')) ?>;
        const monthData = <?= json_encode(array_column($monthlyStats, 'count')) ?>;
